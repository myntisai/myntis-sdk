...
export class DistributorContract extends BaseContract {
  ...
  async submitMerkleRoot(
    root: HexString,
    expiry: number,
    totalClaimableAmount: Wei,
    zkProof?: ZKProof,
    publicInputs?: string[]
  ): Promise<{ tx: TransactionResult; rootIndex: number }> {
    this.requireSigner();

    let tx: TransactionResult;
    if (zkProof && publicInputs) {
      // Submit with ZK proof
      tx = await this.contract.submitMerkleRoot(
        root,
        expiry,
  ...
  async depositBalance(amount: Wei): Promise<TransactionResult> {
    this.requireSigner();
  ...
  // Read operations
  async getProviderBalance(provider: Address): Promise<Wei> {
    return await this.contract.getProviderBalance(provider);
  }

  async getLockedBalance(provider: Address): Promise<Wei> {
